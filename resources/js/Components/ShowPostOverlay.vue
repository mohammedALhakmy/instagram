...
const emit = defineEmits(['closeOverlay', 'addComment', 'updateLike', 'deleteSelected']);
        ...
        <div class="max-w-6xl h-[calc(100%-100px)] mx-auto mt-10 bg-white rounded-xl">
            <div class="w-full md:flex h-full overflow-auto rounded-xl">
                <div class="flex items-center bg-black w-full">
                    <img class="rounded-xl min-w-[400px] p-4 mx-auto" :src="post.file" alt="">
                </div>

                <div class="md:max-w-[500px] w-full relative">
                    <div class="flex items-center justify-between p-3 border-b">
                        <div class="flex items-center">
                            <img class="rounded-full w-[38px] h-[38px]" :src="post.user.file" alt="">
                    ...
                    <div class="overflow-auto h-[calc(100%-170px)]">
                        ...
                        <div  v-if="post.comments"
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex items-center">
                                    <img class="rounded-full w-[38px] h-[38px]" :src="comment.user.file" alt="">
                                    <div class="ml-4 font-extrabold text-[15px]">
                                        <span class="font-extrabold text-[15px] mr-2">
                                            {{ comment.user.name }}
                                        </span>
                                        <span class="text-[15px] text-gray-900">{{ post.created_at }}</span>
                                    </div>
                                </div>
                                <DotsHorizontal
                        ...
                        <LikesSection class="px-2 border-t mb-2" v-if="post" :post="post" @like="$emit('updateLike',$event)"/>
                        <div class="absolute flex border bottom-0 w-full max-h-[200px] bg-white overflow-auto">
                        <EmoticonHappyOutline class="pl-3 pt-[18px]" :size="30"/>
                        <textarea ref="textarea" :oninput="textareaInput" v-model="comment"  class="w-full border-0
                         mt-2 mb-2 text-sm z-50 focus:ring-0 text-[18px]" placeholder="add commit.." rows="1"></textarea>
                        <button
                            v-if="comment"
