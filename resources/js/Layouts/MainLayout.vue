    ...
    <div id="MainLayout" class="w-full h-screen">
        <div id="TopNavHome" class="fixed z-30 md:hidden block w-full bg-white h-[61px] border-b border-b-gray-300">
            <div class="flex items-center justify-between h-full">
                <Link href="/"><img src="/insta-logo.png" class="w-[105px] ml-6 cursor-pointer" alt=""></Link>
                <div class="flex items-center w-[50%]">
                    <div class="flex items-center w-full bg-gray-100 rounded-lg">
                        <Magnify class="pl-4" fill-color="#8E8E8E" :size="27"/>
                        <input type="text" class="bg-transparent w-full placeholder-[#8E8E8E] border-0 ring-0 focus:ring-0" placeholder="Search...">
                    </div>
        ...
        <div id="SideNav" class="fixed h-full bg-white xl:w-[160px] w-[80px] md:block hidden border-r border-r-gray-300">
            <Link href="/">
                <img class="xl:hidden block w-[22px] mt-5 ml-[25px] mb-6 cursor-pointer" src="/insta-logo-small.png" alt="">
                <img class="xl:block hidden w-[100px] mt-5 ml-6 mb-6 cursor-pointer" src="/insta-logo.png" alt="">
            </Link>
            <div class="px-3 w-[100%]">
                <Link href="/">
                    <MenuItem iconString="Home"/>
                </Link>
                <MenuItem iconString="Search"/>
                ...
                <MenuItem @click="$event => showCreatePost = true" icon-string="Create"/>
        ...
        <div class="flex lg:justify-between bg-white h-full w-[100%-280px] lg:pl-[100px] overflow-y-auto">
            ...
            <div v-if="$page.url === '/'" id="SuggestionSection" class="lg:w-4/12 lg:block hidden text-black mt-10">
                <Link :href="route('users.show',{id:$page.props.auth.user.id})" class="flex items-center justify-between max-w-[300px]">
                    <div class="flex items-center">
                        <img :src="$page.props.auth.user.file" class="rounded-full z-10 w-[37px] h-[37px]" alt="">
                        <div class="pl-4">
                    ...
                    <button class="text-blue-500 hover:text-gray-900 text-xs font-extrabold">Switch</button>
                </Link>
                ...
                <div v-for="randUser in $page.props.randomUsers" :key="randUsers" class="">
                    <Link :href="route('users.show',{id:randUser.id})" class="flex items-center justify-between max-w-[300px] pb-2">
                        <div class="flex items-center">
                            <img :src="randUser.file" class="rounded-full z-10 w-[37px] h-[37px]" alt="">
                            <div class="pl-4">
                                <div class="text-black font-extrabold">{{randUser.name}}</div>
                                <div class="text-gray-500 text-extrabold text-sm">Suggested For you5</div>
                            </div>
                        </div>
                        <button class="text-blue-500 hover:text-gray-900 text-xs font-extrabold">Follow</button>
                    </Link>
                </div>

                <div class="max-w-[300px] mt-5">
                    <div class="text-sm text-gray-400">About Help Press API Jobs Privacy Terms Location Language Meat Verfied</div>
                    <div class="text-left text-gray-400 mt-4">@2023 INSTAGRM FROM META</div>
                </div>
            </div>
        </div>
        <div class="fixed z-30 bottom-0 w-full md:hidden flex items-center justify-around bg-white border-t py-2 border-t-gray-300" id="BottonNav">
            <Link href="/">
                <HomeOutline fill-color="#000000" :size="33" class="cursor-pointer"/>
            </Link>
            <Compass fill-color="#000000" :size="33" class="cursor-pointer"/>
            <SendOutline fill-color="#000000" :size="33" class="cursor-pointer"/>
            <Plus @click="$event=>showCreatePost = true" fill-color="#000000" :size="33" class="cursor-pointer"/>
            <AccountOutline fill-color="#000000" :size="33" class="cursor-pointer"/>
            <Link :href="route('users.show',{id: $page.props.auth.user.id})">
